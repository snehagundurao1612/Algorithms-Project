...
public class kthSmallest3 {
	...
	public static int ksmallest(int[] s, int firstIndex, int lastIndex, int k) {
		...
		if (n >= 3) {
			...
			if ((i - 1) * 3 < n) {
				Quicksort.quickSort(s, ((i - 1) * 3), lastIndex);
				medians[i - 1] = s[((i - 1) * 3) + (int) Math.ceil(((n - 1) - ((i - 1) * 3)) / 2)];
	...
	public static void main(String arg[]) throws NumberFormatException, IOException {
		FileReader inputfile = new FileReader("D://Algorithms//kthSmallest100000Set1.txt");
		...
		File file = new File("D://Algorithms//outputkthsmallest3.txt");
